...
Feature: [SWAGGER-5] Обновление информации о питомце
  ...
  @SWAGGER-5.4 @negative
  Scenario Outline: Обновление информации о питомце (указание невалидного значения поля status)
    Given формируем JSON на основе шаблона "addNewPet.json" и сохраняем в Memory как "request_body"
      | field         | value                    |
      | pet_entity_id | GENERATE : pet_entity_id |
      | category_id   | GENERATE : id            |
    ...
    Examples:
      | status  | code | phrase               |
      | null    | 405  | Validation exception |
      | created | 405  | Validation exception |