...
Feature: [SWAGGER-6] Удаление записи о заказе
  ...
  Scenario Outline: Удаление записи о существующем заказе
    ...
    When отправляем DELETE запрос на "https://petstore.swagger.io" эндпойнт "/v2/store/order/" и сохраняем ответ в Memory как "response_entity"
      | variable      | value             |
      | pet_entity_id | MEMORY : order_id |
